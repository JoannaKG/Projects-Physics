...
def predict_fitting_parameters(filepath, n_peaks_to_find=9, delta_E=0.001):
    ...
    filename = 'Figures/Report.txt'  # output reports
    os.makedirs(os.path.dirname(filename), exist_ok=True)
    file = open(filename, 'w')  # file with reports: successed/not succesed fitting for each column
    cols = ['parameters']
    ...
    file.close()
    out_df.columns = cols
    writer = 'Figures/Params_extracted.csv'
    os.makedirs(os.path.dirname(writer), exist_ok=True)
...
def plot_fitted_curves(filepath, df_fitted_parameters):
    ...
    for col in range(1, number_of_columns):
        if col == df_fitted_parameters.columns[col]:
            ...
            filename = 'Figures/E_exc' + str(round(df_fitted_parameters.loc[25, col], 4)) + ' eV.png'  # output reports
            os.makedirs(os.path.dirname(filename), exist_ok=True)
            #plt.savefig('E_exc = ' + str(round(df_fitted_parameters.loc[25, col], 4)) + ' eV.png')
...
if __name__ == "__main__":
    out = predict_fitting_parameters(path)
    plot_fitted_curves(path, out)
