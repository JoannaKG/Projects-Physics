...
def intensity(TMD, d_SiO2=300, d_hBN=20, lam_0=514, lambda_min = 360, lambda_max = 1000, number_of_layers = 1, th_0=0, dtype=complex): 
    ...
    d_hBN_top=0
    def t_prim(lam, dtype=complex):
        t_prim=(t("hBN","air",lam)*t("MoS2","hBN",lam)*exp(-1j*beta_j(d_hBN_top,"hBN",lam)))/(1+r("air", "hBN",lam)*r("hBN","MoS2",lam)*exp(-2j*beta_j(d_hBN_top,"hBN",lam)))
        return t_prim
    def t_bis(lam, dtype=complex):
        t_prim=(t("air","hBN",lam)*t("hBN","MoS2",lam)*exp(-1j*beta_j(d_hBN_top,"hBN",lam)))/(1+r("air", "hBN",lam)*r("hBN","MoS2",lam)*exp(-2j*beta_j(d_hBN_top,"hBN",lam)))
        return t_prim
