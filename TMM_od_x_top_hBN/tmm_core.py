...
def coh_tmm(TMD, number_of_layers, pol, n_list, d_list, th_0, lam_vac, n_list_abs, lam0=514):
    ...
    t_list[2,1]=interface_t(pol, n_list[2], n_list[1],th_list[2], th_list[1])
    r_list[1,0]=interface_r(pol, n_list[1], n_list[0],th_list[1], th_list[0])

    t_prim=(t_list[1,0]*t_list[2,1]*exp(1j*delta[1]))/(1+r_list[0,1]*r_list[1,2]*exp(2j*delta[1]))
    r_tri=(r_list[1,2]+r_list[0,1]*exp(2j*delta[1]))/(1+r_list[0,1]*r_list[1,2]*exp(2j*delta[1]))
    
    #print('t_prim_sc='+str(t_prim)) 
    ...
    def F_sc(x,Mtilde1, dtype=complex):
        ...
        F_sc_arr = np.dot(make_2x2_array(1, r_tri, t_prim*exp(1j*beta_x(x,lam_vac)), t_prim*exp(-1j*beta_x(x, lam_vac)),
    ...
    r_list_abs[1,0]=interface_r(pol, n_list_abs[1], n_list_abs[0],th_list_abs[1], th_list_abs[0])
    t_list_abs[1,0]=interface_t(pol, n_list_abs[1], n_list_abs[0],th_list_abs[1], th_list_abs[0])
    t_list_abs[2,1]=interface_t(pol, n_list_abs[2], n_list_abs[1],th_list_abs[2], th_list_abs[1])
    
    r_tri_abs=(r_list_abs[1,2]+r_list_abs[0,1]*exp(2j*delta_abs[1]))/(1+r_list_abs[0,1]*r_list_abs[1,2]*exp(2j*delta_abs[1]))
    t_prim_abs=(t_list_abs[0,1]*t_list_abs[1,2]*exp(1j*delta_abs[1]))/(1+r_list_abs[0,1]*r_list_abs[1,2]*exp(2j*delta_abs[1]))
    
    #print('t_prim_abs='+str(t_prim_abs))  
    ...
    def F_abs(x,Mtilde_abs, dtype=complex):
        ...
        F_abs_arr = np.dot(make_2x2_array(1, r_tri_abs, t_prim_abs*exp(1j*beta_x(x, lam0)), t_prim_abs*exp(-1j*beta_x(x, lam0)),
 ...
 #---------------liczenie całki-----------------------------   
    def integrand(x):
            ...
            #return (abs(F_sc(x,Mtilde1)))**2  
        #(F_sc(x,Mtilde1)).real
    ans, err = quad(integrand,0, layer_thickness)
    Enh=ans#/layer_thickness
    #print(str(ans))
